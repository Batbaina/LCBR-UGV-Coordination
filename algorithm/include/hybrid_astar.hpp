 ...
 * @brief Spatiotemporal Hybrid-State A* (SHA*) search engine.
 *
 * Reused unmodified from CL-CBS (Wen et al., 2022). This is the exact
 * search engine invoked by both the full-horizon low-level query (CL-CBS
 * baseline) and the bounded local query (LCBR, this project's
 * contribution): motion primitives, cost function g(.), heuristic and
 * analytic expansion are all identical between the two. Only the
 * `Environment` object passed in (start/goal boundary configuration and
 * active constraint set) differs -- see local_repair.hpp.
 *
 * @copyright Original (c) 2020 Licheng Wen (wenlc@zju.edu.cn), Zhejiang
 * University. Redistributed here under the project's original license
 * (see LICENSE) as an unmodified dependency of UGV-Coordination-System.
 */
...
class HybridAStar {
 public:
  ...
  HybridAStar(Environment& environment, size_t maxExpansions)
      : m_env(environment), m_maxExpansions(maxExpansions) {}
  ~HybridAStar() {}
  ...
  bool search(const State& startState,
    ...
    // [LCBR/robustness] The original CL-CBS low-level search has no
    // built-in cap on the number of expansions: on a genuinely hard or
    // practically-infeasible single robot->goal query (this can and does
    // happen on Wen et al.'s own benchmark -- a query whose only path
    // requires a maneuver the vehicle's minimum turning radius cannot
    // execute, for instance), `s.time` increments forever with no bound
    // and the open set grows without limit until the process is
    // OOM-killed. Capping expansions and returning failure past the cap
    // is the operational reading of the paper's own Eq. 6 ("Gamma_ij = 
    // empty set denotes an infeasible planning query"): a query that does
    // not terminate within a generous low-level budget is treated as
    // infeasible for Stage 1/2/3's purposes, exactly like a query that
    // returns no path for any other reason. Default is effectively
    // unbounded (matches the original code's behavior) unless the caller
    // opts in via the two-argument constructor.
    size_t expansions = 0;

    while (!openSet.empty()) {
      if (m_maxExpansions > 0 && expansions >= m_maxExpansions) {
        return false;
      }
 ...
 private:
  Environment& m_env;
  size_t m_maxExpansions = 0;  // 0 = unbounded (original CL-CBS behavior)
};
